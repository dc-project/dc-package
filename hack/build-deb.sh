...
(
    ...
    do
        ...
        RELEASE_PATH=$debbuild_root/$release/${PROGRAM}-${project}-${debbuildRelease}
        ...
        docker run -rm -v $PWD/$debbuild_root/$release:/debbuild -w /debbuild/${PROGRAM}-${project}-${debbuildRelease} -e VERSION=$VERSION -e debRelease=$debbuildRelease $BUILD_IMAGE build
    done
) 2>&1